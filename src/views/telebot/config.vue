<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column label="配置ID" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="配置名" prop="id" align="center" width="120">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="配置值" prop="id" align="center" width="120">
        <template slot-scope="{row}">
          <span>{{ row.value }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item v-if="dialogStatus==='create'" label="配置键值">
...
import { getList, update, del, add } from '@/api/telebot/config'
