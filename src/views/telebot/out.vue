  ...
  <div class="app-container">
    ...
    >
      <el-table-column label="用户ID" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.uid | userFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="申请金额" prop="id" align="center" width="120">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作区" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-button v-if="row.status !== 7" size="mini" type="danger" @click="handleDelete(row,$index)">
            删除申请
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item v-if="temp.status ===4" label="失败理由">
...
import { getUser, getList, update, del } from '@/api/telebot/out'
...
export default {
  ...
  created() {
    getUser(new Date().getTime()).then(res => {
      this.getList()
      userOption = [{ uid: 0, sid: '未知用户' }]
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.statusOpt = []
      this.statusOpt.push({ id: 3, name: '下发成功' })
      this.statusOpt.push({ id: 4, name: '下发失败' })
      this.temp.status = 3
